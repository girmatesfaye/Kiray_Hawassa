  ...
  return (
    <View className="flex-1 bg-white pt-8 justify-between">
      <View className="flex-1">
        <HeaderBar title="Post Property - Step 3/3" subtitle="Commission & Agreement" showBack />
        ...
        <ScrollView className="p-4 flex-1">
          ...
          <Text className="text-sm text-gray-600 mb-6 leading-6">
            Kiray Hawassa operates on a verified local agent connector model to ensure high trust, safety, and rapid deal closure.
          </Text>

          {/* Commission Highlight Box */}
          <View className="bg-emerald-50 border border-emerald-200 p-5 rounded-2xl mb-6">
            <Text className="text-xs font-bold text-emerald-800 uppercase tracking-wider mb-1">Standard Rate</Text>
            <Text className="text-3xl font-extrabold text-emerald-900 mb-1">10% First Month</Text>
            <Text className="text-xs text-emerald-700">
              Only payable upon successful lease agreement and tenant deposit collection.
            </Text>
          </View>
          ...
          >
            <View className={`w-6 h-6 rounded-md items-center justify-center mr-3 ${acceptedTerms ? 'bg-emerald-700' : 'border border-gray-400'}`}>
      ...
      <View className="p-4 border-t border-gray-100 bg-white">
        <TouchableOpacity
          ...
          className={`py-4 rounded-xl items-center justify-center shadow-sm ${
            acceptedTerms ? 'bg-emerald-700' : 'bg-gray-200'
          }`}
        >
          <Text className={`text-base font-bold ${acceptedTerms ? 'text-white' : 'text-gray-400'}`}>
